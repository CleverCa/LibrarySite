...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($confirm_password)) {
        $errors[] = "Поле подтверждения пароля обязательно для заполнения";
    }
    ...
    $check_query = "SELECT ReaderID FROM readers WHERE login = ?";
    if (!empty($user_id)) {
        $check_query .= " AND ReaderID != ?";
    ...
    if (empty($errors)) {
        if (!empty($user_id)) {
            ...
            $update_query = "UPDATE readers SET
                                      ...
                                      login = ?,
                                      password = ?";
            $params = [$first_name, $last_name, $middle_name, $phone, $email, $login, password_hash($password, PASSWORD_BCRYPT)];
            $types = "sssssss";
        } else {
            // Добавление нового пользователя
            $update_query = "INSERT INTO readers (FirstName, LastName, MiddleName, Phone, Email, login, password) VALUES (?, ?, ?, ?, ?, ?, ?)";
            $params = [$first_name, $last_name, $middle_name, $phone, $email, $login, password_hash($password, PASSWORD_BCRYPT)];
            $types = "sssssss";
        }
